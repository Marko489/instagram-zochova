...
const Post: React.FC<PostProps> = ({ post }) => {
  ...
  const { data: session } = useSession();
  const [likes, setLikes] = useState(post.likes.length);
  const [isLiked, setIsLiked] = useState(post.likes.some((like: Like) => like.userId === session?.user.id));
  const [comments, setComments] = useState(post.comments);
  const [newComment, setNewComment] = useState('');
  const [inlineNewComment, setInlineNewComment] = useState('');
  const [showPopup, setShowPopup] = useState(false);
  const [isBookmarked, setIsBookmarked] = useState(post.bookmarks.some((bookmark: Bookmark) => bookmark.userId === session?.user.id));
  const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        const updatedPost = response.data;
        setLikes(updatedPost.likes.length);
        setIsLiked(updatedPost.likes.some((like: Like) => like.userId === session?.user.id));
        setComments(updatedPost.comments);
      } catch (error) {
        console.error('Error fetching updated post:', error);
      }
  ...
  const handleAddComment = async (content: string, setInputValue: React.Dispatch<React.SetStateAction<string>>) => {
    ...
    try {
      ...
      setComments([...comments, {
        ...
        content: addedComment.content,
        userId: addedComment.userId,
        user: { name: session?.user.name ?? 'Unknown User' } // Ensure name is a string
      }]);
